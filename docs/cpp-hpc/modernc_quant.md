...
> Applicable to StratCraft Executor (C++23 + pybind11 embedded Python).
...
- **Quant Value**: Cache-friendly layout for vectorized operations and efficient NumPy conversion. *(Used in StratCraft Executor)*

### 18. Vector Reserve + Push Pattern

- **Feature**: Pre-allocate vector capacity before bulk insertion.
- **Quant Value**: Eliminates reallocations and iterator invalidation during data loading. *(Used in StratCraft Executor)*
...
- **Quant Value**: Monotonic, high-resolution timing immune to system clock adjustments. *(Used in StratCraft Executor)*
...
- **Quant Value**: Prevents accidental duplication of interpreter or connection state. *(Used in StratCraft ExecutorCore)*

### 37. `std::optional<T>` Nullable Fields

- **Feature**: Explicit optional semantics for configuration fields.
- **Quant Value**: Type-safe nullability without sentinel values or pointers. *(Used in StratCraft config_types.hpp)*
...
- **Quant Value**: Type-safe formatting without printf vulnerabilities. *(Used in StratCraft Executor)*
...
- **Quant Value**: Single codebase for Windows/Linux/macOS file handling. *(Used in StratCraft Executor)*
...
- **Quant Value**: Market data passed to Python without any memory copy. *(Core pattern in StratCraft Executor)*

### 47. Embedded Interpreter Lifecycle (RAII)

- **Feature**: Initialize/finalize Python interpreter in constructor/destructor.
- **Quant Value**: Deterministic resource management prevents interpreter leaks. *(Used in StratCraft ExecutorCore)*

### 48. Mutex-protected Global State

- **Feature**: Use `std::mutex` + `std::lock_guard` for shared Python state.
- **Quant Value**: Thread-safe access to global DataFrame/config during concurrent operations. *(Used in StratCraft bindings.cpp)*

### 49. `std::function` Progress Callbacks

- **Feature**: Type-erased callbacks for progress/increment reporting.
- **Quant Value**: Decouples executor from UI layer while enabling real-time updates. *(Used in StratCraft Executor)*

### 50. Factory Pattern for Data Sources

- **Feature**: `createDataSource()` dispatches to Parquet/Mock implementations.
- **Quant Value**: Pluggable data loaders without modifying core execution logic. *(Used in StratCraft Executor)*
...
- **Quant Value**: Read only required columns; zero-copy to NumPy arrays. *(Used in StratCraft ParquetDataSource)*

### 52. nlohmann/json Compile-time Parsing

- **Feature**: JSON serialization with type-safe macros.
- **Quant Value**: Automatic struct-to-JSON mapping without runtime reflection. *(Used in StratCraft config/result types)*

### 53. Move Semantics for DataFrame Transfer

- **Feature**: Use `std::move()` for DataFrame ownership transfer.
- **Quant Value**: Zero-copy handoff between data source and executor core. *(Used in StratCraft Executor)*

### 54. Static Linking for Portability

- **Feature**: Link libgcc/libstdc++ statically.
- **Quant Value**: Single binary runs across Linux distributions without dependency issues. *(Used in StratCraft CMake)*

### 55. Time-range Filtering Post-load

- **Feature**: Filter data after loading based on execution config.
- **Quant Value**: Avoids re-reading Parquet files for different backtest periods. *(Used in StratCraft ParquetDataSource)*
...
- [V3 Architecture](../design/_V3_ARCHITECTURE_REFACTORING.md)
