...
const SignalGeneratorPlaceholder: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex items-center justify-center">
      <div className="text-center max-w-md px-6">
        <Activity className="w-16 h-16 mx-auto mb-6 text-color-terminal-accent-primary opacity-40" />
        <h2 className="text-xl font-semibold text-color-terminal-text mb-3">
          {t('signalGenerator.title')}
        </h2>
        <p className="text-color-terminal-text-secondary mb-6 leading-relaxed">
          {t('signalGenerator.pluginDescription')}
        </p>
        <button
        ...
        >
          ...
          {t('signalGenerator.openMarketplace')}
        </button>
      </div>
    </div>
  );
...
export const SignalGeneratorHostPage: React.FC = () => {
  const { t } = useTranslation('ui');
  const breadcrumbRightContent = useMemo(() => {
    const key = VIEW_REGISTRY.signalGenerator.shortLabelKey;
    const fallback = VIEW_REGISTRY.signalGenerator.shortLabel ?? '';
  ...
  return (
    <div className="flex flex-col h-full bg-StratCraftsAI terminal-theme">
      ...
      <ViewContainer
        viewId="signalGenerator.main"
        fallback={<SignalGeneratorPlaceholder />}
      />
    </div>
  );
};

export default SignalGeneratorHostPage;
