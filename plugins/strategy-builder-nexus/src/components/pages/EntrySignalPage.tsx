...
import {
  ...
  buildRegimeIndicatorEntryRequest,
...
import type { Strategy } from './useEntrySignalStore';
...
async function buildStorageRequestFromResult(
  ...
  const request = buildEntrySignalRequest(
    {
      strategy_name: strategyName,
...
export const EntrySignalPage: React.FC<EntrySignalPageProps> = ({
  ...
  useEffect(() => {
    setPageTitle(t('pages.entrySignal.breadcrumb'));
    return () => setPageTitle(null);
  }, [setPageTitle, t]);

  // ---------------------------------------------------------------------------
  // Page-specific State (UI inputs) — TICKET_1208 P6: Zustand store
  // ---------------------------------------------------------------------------
  ...
  const currentState: EntrySignalState = useMemo(() => ({
    ...
    llmModel,
    signalMode,
  }), [selectedRegime, indicatorBlocks, strategies, llmProvider, llmModel, signalMode]);
  ...
  const workflowConfig = useMemo((): GenerateWorkflowConfig<RegimeIndicatorEntryConfig, EntrySignalState> => ({
    ...
    executeApi: executeRegimeIndicatorEntry as (config: RegimeIndicatorEntryConfig, signal?: AbortSignal) => Promise<GenerationResult>,
    buildGenerationRequest: buildRegimeIndicatorEntryRequest,
  ...
  const handleNewStrategy = useCallback(async () => {
    ...
    if (confirmed) {
      ...
      useEntrySignalStore.getState().reset();
    }
  }, [actions, t]);
